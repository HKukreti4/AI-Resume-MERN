...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="flex mt-3 items-center justify-between px-6 py-3 md:py-4 shadow max-w-4/5 rounded-full mx-auto w-full bg-gray-700/30 ">
      <Logo/>

      <nav
        id="menu"
        className={` max-md:top-0 max-md:left-0 max-md:overflow-hidden items-center justify-center max-md:h-full transition-[width] text-white backdrop-blur flex-col md:flex-row flex gap-8  text-sm font-normal ${
          isMenuOpen ? "max-md:w-full fixed bg-black  top-0 left-0" : "max-md:w-0 max-md:absolute"
        }`}
      >
